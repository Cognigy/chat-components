...
Current entries: one for `"stateful: datepicker open dialog"` — `aria-required-children`, tracked under **AB#144248**. The flatpickr day cells must stay flat DOM children of `.dayContainer` (flatpickr indexes them by position for arrow navigation and range hover), so the `role="row"` level is supplied by hidden row elements that claim their cells through `aria-owns`. Browsers give `aria-owns` precedence over DOM parentage, so the accessibility tree is `grid > rowgroup > row > gridcell`; axe's `aria-required-children` check does not model that precedence and still counts the cells under the rowgroup. This entry documents a tooling limitation rather than markup debt (the former `aria-required-parent` and `label` entries were cleared by the same change).
